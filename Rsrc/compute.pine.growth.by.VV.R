...
# Bonity class
H100 <- 30
# H101 = H100 except for H100 = 30, then H101 = 27
H101 <- 27
...
PH.pine.f <- function(H, T, a0 = -0.40006, a1 = 434.52, a2 = -124.51) {
	    X <- 1 / (H^0.4 * T^1.1)
	    PH <- a0 + a1 * X + a2 * X^2
	    return(PH)}
...
PG.pine.f <- function(H101, H, T, Gu, a0 = -0.89537, a1 = 0.0019059, a2 = 730.26 , a3 = 363780) {	    
            alfa <- 0.8 + H101 / 175
...
PV.pine.f <- function(H100, H, T, Vu, a0 = -0.95493, a1 = 0.068517, a2 = 17838 , a3 = 7165400) {
	           X <- 1 / (Vu^0.55*T^1.45)
...
FH.pine.f <- function(G, T, H, a0 = 0.49372, a1 = -0.0016280, a2 = 4.4746) {	    
            FH <- a0 + a1 * H + a2 * (1/(T * G))
	    return(FH)}
...
RG.pine.f <- function(H, RV, a0 = 2.1070, a1 = -0.078991, a2 = 1.0126) {
	    RG <- a0 + a1 * H + a2 * RV
	    return(RG)}
...
RN.pine.f <- function(RV, N, G, V,
	    a0 = -0.25627, a1 = 1.1320, a2 = 0.26511, a3 = -0.86598) {
...
BGu.pine.f <- function(Gu, H, T, a0 = 0.67175, a1 = 0.086641, a2 = -0.013987, a3 = 0.15393) {
...
BG.pine.f <- function(G, H, T ,  a0 = 0.67175, a1 = 0.086641, a2 = -0.013987, a3 = 0.15393) {
...
BV.pine.f <- function(H , T, a0 = 3.9251, a1 = -0.19666, a2 = -0.17774, half.MSE = 0.0137) {
...
Sperc.pine.f <- function(V, N, T, a0 = 0.42486, a1 = -0.0051294, 
	       a2 = 0.0037506, half.MSE = 0.019969) {
...
Wperc.pine.f <- function(V, N, a1 = 0.014317) {
	       Wperc <- 21.838 * a1 * (590.559 / (1000 * V/N - 9.8)^0.9 - 1) + 0.174
	       return(Wperc)}
...
for ( i in 2:length(Ttemp) ) {
       Htemp[i] <- Htemp[i-1] * (5 * PH.pine.f(H = Htemp[i-1], T = Ttemp[i-1]) / 100 + 1)   }   
...
Bperc[1] <- 100 * ( V[1] - BV.pine.f(H = H[1], T = T[1]) ) / V[1]
vbar <- vector()
...
# stated in the text):
Ts <- c(30, 35, 35, 45, 50, 70)[c(30, 27, 24, 21, 18 , 15) %in% H100]
...
length(S) <- length(T)
{if(T0 < Ts) S[1] <- 0
else S[1] <- max(0, V[1] * Sperc.pine.f(V = V[1], N = N[1], T = T[1]) / 100)}
...
length(FF) <- length(T)
{if(T0 < Ts) FF[1] <- max(0, V[1] * ( 100 - Wperc.pine.f(V = V[1], N = N[1]) ) / 100)
else FF[1] <- max(0, V[1] * ( 100 - Sperc.pine.f(V = V[1], N = N0, T = T[1]) -
       	    Wperc.pine.f(V = V[1], N = N[1]) ) / 100)}
...
for ( i in 2:length(T) ) {
       Gu[i] <- Gu[i-1] * (5 * PG.pine.f(H101 = H101 , H = H[i-1], T = T[i-1], Gu = Gu[i-1]) / 100 + 1)
       G[i] <- Gu[i] + BGu.pine.f(Gu = Gu[i], H = H[i], T = T[i])
       V[i] <- H[i] * G[i] * FH.pine.f(G = G[i], T = T[i], H = H[i])
       Bperc[i] <- 100 * ( V[i] - BV.pine.f(H = H[i], T = T[i]) ) / V[i]
       vbar[i] <- 1000 * V[i] / N[i]
       {       if(T[i] < Ts) {
       	       S[i] <- 0
	       FF[i] <- V[i] * ( 100 - Wperc.pine.f(V = V[i], N = N[i]) ) / 100 
      }
       else {
       	    S[i] <- V[i] * Sperc.pine.f(V = V[i], N = N[i], H = H[i]) / 100
	    FF[i] <- V[i] * ( 100 - Sperc.pine.f(V = V[i], N = N[i], T = T[i]) - 
...
# Time step of the first thinning
	 ...
	 vbar[i] <- 1000 * V[i] / N[i]
   {
	 	 ...
	 	 FF[i] <- V[i] * ( 100 - Wperc.pine.f(V = V[i], N = N[i]) ) / 100	 }
       	      ...
       	      S[i] <- V[i] * Sperc.pine.f(V = V[i], N = N[i], T = T[i]) / 100	 
     FF[i] <- V[i] * ( 100 - Sperc.pine.f(V = V[i], N = N[i], T = T[i]) -
	      	   Wperc.pine.f(V = V[i], N = N[i]) ) / 100         } 
      	 }
     }
     N[i1] <- (1 - RN.pine.f(RV = thin.ints[1] * 100, N = N[i1-1], G = G[i1-1], V = V[i1-1]) / 100) * N[i1-1]
     G[i1] <- (1 - RG.pine.f(H = H[i1-1], RV = thin.ints[1] * 100) / 100) * G[i1-1]
     Gu[i1] <- G[i1] - BG.pine.f(G = G[i1], H = H[i1], T = T[i1])
     V[i1] <- (1 - thin.ints[1]) * V[i1-1]
     Bperc[i1] <- 100 * ( V[i1] - BV.pine.f(H = H[i1], T = T[i1]) ) / V[i1]
     vbar[i1] <- 1000 * V[i1] / N[i1]
      {     if(T[i1] < Ts) {     	     S[i1] <- 0	
     FF[i1] <- V[i1] * ( 100 - Wperc.pine.f(V = V[i1], N = N[i1]) ) / 100     }
     else {
     	  S[i1] <- V[i1] * Sperc.pine.f(V = V[i1], N = N[i1], T = T[i1]) / 100
	  FF[i1] <- V[i1] * ( 100 - Sperc.pine.f(V = V[i1], N = N[i1], T = T[i1]) -
...
# Time step of the current thinning
	  ...
	  N[i2] <- (1 - RN.pine.f(RV = thin.ints[j] * 100, 
     	       N = N[i2-1], G = G[i2-1], V = V[i2-1]) / 100) * N[i2-1]
	  G[i2] <- (1 - RG.pine.f(H = H[i2-1], RV = thin.ints[j] * 100) / 100) * G[i2-1]
	  Gu[i2] <- G[i2] - BG.pine.f(G = G[i2], H = H[i2], T = T[i2])
	  V[i2] <- (1 - thin.ints[j]) * V[i2-1]	  
          Bperc[i2] <- 100 * ( V[i2] - BV.pine.f(H = H[i2], T = T[i2]) ) / V[i2]
	  vbar[i2] <- 1000 * V[i2] / N[i2]
       	  {
       	  if(T[i2] < Ts) {
       	  	  S[i2] <- 0
	 	  FF[i2] <- V[i2] * ( 100 - Wperc.pine.f(V = V[i2], N = N[i2]) ) / 100
	  }
       	  else {
       	       S[i2] <- V[i2] * Sperc.pine.f(V = V[i2], N = N[i2], T = T[i2]) / 100
	       FF[i2] <- V[i2] * ( 100 - Sperc.pine.f(V = V[i2], N = N[i2], T = T[i2]) -
...
rm(H100, H101, T0, H0, N0, G0, Tn, thin.times, thin.ints, thin.n, Hsource, Ts, i,
	 T, H, N, G, Gu, V, Bperc, vbar, S, FF,	 df.growth)

rm(BG.pine.f, BGu.pine.f, BV.pine.f, FH.pine.f, PG.pine.f,   
PH.pine.f, PV.pine.f, RG.pine.f, RN.pine.f,   
		Sperc.pine.f, Wperc.pine.f)